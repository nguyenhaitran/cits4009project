...
```{r}
region_patterns <- grepl("(WB)|(WHO)|^World|^G20", data$Entity)
...
```{r}
wb_mask<-grepl("(WB)", regions$Entity)
who_mask<-grepl("(WHO)", regions$Entity)
world_mask<-grepl("World", regions$Entity)
world_bank_mask<-grepl("World Bank", regions$Entity)
G20_mask<-grepl("G20", regions$Entity)
```

```{r}
wb_data<-regions[wb_mask,]
wb_data
```
...
```{r}
who_data<-regions[who_mask,]
who_data
```
```{r}
world_data<-regions[world_mask,]
world_data
```
```{r}
world_bank_data<-regions[world_bank_mask,]
world_bank_data
```


```{r}
g20_data<-regions[G20_mask,]
g20_data
```
