...
```{r}
total_countries_death$Label<-ifelse(total_countries_death$Income.group == "High income", 1, 0)
total_countries_death$Label<-as.factor(total_countries_death$Label)
total_countries_death
```
```{r}
#check if there is any NA values in the data
sum(is.na(total_countries_death))
total_countries_death[!complete.cases(total_countries_death),]
```
...
```{r}
total_countries_death <- na.omit(total_countries_death)
```
...
responses<- colnames(total_countries_death) %in% c("Entity", "Income.group", "Label")
...
NOTE: NEED TO DO ROC PLOT FOR MODEL EVALUATION
...
```{r}
log_single_model<-caret::train(x=train_data["Smoking"], y=train_data$Label, method="glm", family = binomial(link="logit"), metric="Accuracy")
log_single_model_prediction<-predict(log_single_model, test_data["Smoking"])
explainer_single_model<-lime(train_data["Smoking"], model=as_classifier(log_single_model), bin_continuous=TRUE, n_bins=10)
```
