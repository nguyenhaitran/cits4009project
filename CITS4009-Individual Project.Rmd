...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            selectInput("region", "Choose country/region:", choices = unique(data$Entity)),
            selectInput("reason", "Choose x for Histogram 1:", choices = names(data)[4:31])
        ),
...
server <- function(input, output){
    ...
    output$histogram <- renderPlot({
      region_data<-data[data$Entity==input$region,]
