...
With the optimal k is 3, all the data points were divided into 3 groups: 
- World and G20
- Low-income, African, South and South-East Asian countries
- High income countries and other regions which seems to be developed countries.
...
## Shiny App
For every graphs coverd in this report, it will all be put in this Shiny app. This Shiny app includes:
- Exploratory Data Analysis
- Decision Tree Classification Modelling
- Logistic Regression Classification Modelling
- ROC plots to evaluate AUC
- Clustering
...
#Tab2: Decision Tree Classification Model
...
#Tab3: Logistic Regression Model
...
# Conclusion
To sum up everything, we have done the analysis of Countries' numbers of death and causes during 1990-2019 with the dataset provided by World Bank.

With the exploratary data analysis, we found out that the dataset includes numbers of death not only in different countries, but also in different groups, regions and the World, which makes the death causes got the numbers recorded vary in large range. Therefore we need to separate the dataset in to countries and regions. 

With the classification modelling, the countries dataset was used to predict whether a country is a high income countries based on different causes. There were 2 types of models were performed with full variables and high quality selected-features, they are decision tree and logistic regression. All the total number of deaths over the period were calculated, and the high-countries label were marked. With the decision tree classification models, the full model performed better than the selected-features model, while compared to logistic regression modelling, the model with selected features performed better than the full model since the full model seems to be overfitted or not enough data for training and testing. While we compared the same numbers of features with 2 types of models, the decision tree classification models predicted better than the logistics regression model. To compare the performance between models, the perfect table including accuracy, precision, f1, recall values, along with ROC plot to compare the AUC were done.

With the clustering, the regions dataset was selected for this task. With the optimal k is 3, all the data points were divided into 3 groups: the first group of World and G20, the second group of countries with low-income, African countries, South and South-East Asia, and the third group of high income countries and other regions.

There are some improvement needed for this project. The main one is how to improve the model to predict better with higher chance of classifying correctly. Along with that, as the data are vary in a large range, the log-transformation could be performed to evaluate the models performance.
