...
We will build a function to create the confusion matrix for both decision tree and logistic regression classifier
```{r}
performanceMeasures <- function(pred, truth, name = "model") {
  # Construct confusion matrix ensuring all levels (0 and 1) are present
  ctable <- table(factor(truth, levels = c(0, 1)), factor(pred > 0.322, levels = c(FALSE, TRUE)))
